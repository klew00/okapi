...
//		+ "</FrontMatter><Body><Raw>\n";
	...
	public void testSegmentedSurroundedByInternalCodes () {
		String snippet = STARTFILENOLB
			...
			+ "<ut Type=\"end\">ec</ut>"
			+ "</Raw></Body></TRADOStag>";
		TextUnit tu = FilterTestDriver.getTextUnit(getEvents(filterSeg, snippet, locESEM), 1);
		assertNotNull(tu);
	...
	public void testOutputNotSegmentedWithDF_ForcingOutSeg () {
		...
		String expected = STARTFILENOLB
			...
			+ "<ut Type=\"end\" Style=\"external\">{/P}</ut>"
			+ "</Raw></Body></TRADOStag>";
		assertEquals(expected, FilterTestDriver.generateOutput(getEvents(filterSeg, snippet, locESEM),
			locESEM, filterSeg.createSkeletonWriter(), filterSeg.getEncoderManager()));
	}
	...
	public void testBasicNoExtractableData () {
		String snippet = STARTFILE
			...
			+ "</Raw></Body></TRADOStag>";
		TextUnit tu = FilterTestDriver.getTextUnit(getEvents(filterSegNotForced, snippet, locESEM), 1);
		assertNull(tu);
	}
	...
	public void testOutputNoExtractableData () {
		String snippet = STARTFILE
			+ " <ut Style=\"external\">some &amp; code</ut>\n\n  <!-- comments-->"
			+ "</Raw></Body></TRADOStag>";
		assertEquals(snippet, FilterTestDriver.generateOutput(getEvents(filterSegNotForced, snippet, locESEM),
			locESEM, filterSeg.createSkeletonWriter(), filterSeg.getEncoderManager()));
	}
	...
	public void testBadlyNestedTags () {
		...
		tu = FilterTestDriver.getTextUnit(getEvents(filterSeg, snippet, locESEM), 2);
		assertNotNull(tu);
		cont = tu.getSource();
...
//		String snippet = STARTFILENOLB
//			+ "Outside <Tu MatchPercent=\"0\"><Tuv Lang=\"EN-US\">Inside</Tuv></Tu>"
//			+ "</Raw></Body></TRADOStag>";
//		TextUnit tu = FilterTestDriver.getTextUnit(getEvents(filterSeg, snippet, locESEM), 1);
//		assertNotNull(tu);
//		TextContainer cont = tu.getSource();
//		
//		assertEquals("Outside [Inside]", fmt.printSegmentedContent(cont, true));
//		assertFalse(tu.hasTarget(locESEM));
//	}
	...
	public void testOutputForTwoTUsWithSegmentation () {
		...
		String expected = STARTFILENOLB
			...
			+ "</Raw></Body></TRADOStag>";
		assertEquals(expected, FilterTestDriver.generateOutput(getEvents(filterSeg, snippet, locESEM), locESEM,
			filterSeg.createSkeletonWriter(), filterSeg.getEncoderManager()));
	}
	...
	public void testOutputSimpleGTEscaped () {
		...
		String expected = STARTFILE
			...
			+ "</Tu>"
			+ "</Raw></Body></TRADOStag>";
		((Parameters)(filterSegNotForced.getParameters())).setEscapeGT(true);
		assertEquals(expected, FilterTestDriver.generateOutput(getEvents(filterSegNotForced, snippet, locESEM),
			locESEM, filterSegNotForced.createSkeletonWriter(), filterSegNotForced.getEncoderManager()));
	}
...
//	}
	...
	public void textDoubleExtractionOriginalAllSegmented () {
		...
		assertTrue(rtc.executeCompare(filterSeg, list, "UTF-8", locENUS, locFRFR));
	}
...
//	@Test
	public void __LOCALTEST_ONLY_testDoubleExtractionPrivateFiles () {
		...
		assertTrue(rtc.executeCompare(filterSegNotForced, list, "UTF-8", locENUS, locJA));
		list.clear();
