...
class SourceView extends Composite {
	...
	private Text        m_edStatus;
	private Button      m_chkPending;
	private SashForm    m_splMain;
	
	private Table                 m_Items;
	private SourceModel           m_ItemsM;
	private SourceDetailsPanel    m_Details;
	
	SourceView (Composite p_Parent,
		int p_nFlags,
		MainForm p_MF,
		Controller p_C)
	{
	...
	private void createContent ()
	{
		...
		m_edStatus = new Text(this, SWT.BORDER);
		m_edStatus.setEditable(false);
		gdTmp = new GridData(GridData.HORIZONTAL_ALIGN_FILL); //.FILL_HORIZONTAL);
		gdTmp.horizontalSpan = 6;
		gdTmp.verticalIndent = 2;
		m_edStatus.setLayoutData(gdTmp);
		
		m_chkPending = new Button(this, SWT.CHECK);
		m_chkPending.setText("Show only pending items");
		gdTmp = new GridData(GridData.HORIZONTAL_ALIGN_FILL);
		gdTmp.horizontalIndent = 4;
		gdTmp.verticalIndent = 2;
		m_chkPending.setLayoutData(gdTmp);
		m_chkPending.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				updatePending();
			}
		});
	...
	/**
	 * Sets the document(s) to show in the Source tab.
	 * @param p_nIndex Index (in the DView) of the document. Use -1 for
	 * no document, and -2 for all documents.  
	 */
