 ...
 */
public final class LDSQueryHighlightExtractor { // DWH 5-7-07 just added "LDS"
	 ...
	 */
	public LDSQueryHighlightExtractor(TermHighlighter highlighter, Query query,
			Analyzer analyzer) throws IOException { // DWH 5-7-07 just added
													// "LDS"
		this.query = query;
		// get terms in query
	...
	public LDSQueryHighlightExtractor() throws IOException { // DWH 5-7-07 just
																// added "LDS"
	}
	...
	private final void getTermsFromPhraseQuery(PhraseQuery query, HashSet terms) {
		...
		if (extractPhrases) {
			...
			terms.add(new ArrayList(Arrays.asList(queryTerms))); // DWH 6-7-07
			return;
		}
