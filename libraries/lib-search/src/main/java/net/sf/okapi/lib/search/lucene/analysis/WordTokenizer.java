 ...
 */
public class WordTokenizer extends org.apache.lucene.analysis.Tokenizer
		implements LdsTokenizer, WordTokenizerConstants {
	...
	public void setReader(Reader p_reader) {
		WordTokenizerTokenManager tm = new WordTokenizerTokenManager(
				new FastCharStream(p_reader));
		ReInit(tm);
		// FIXME: Just to get to compile - uncomment and fix
		//this.input = p_reader;
	}
	...
	public org.apache.lucene.analysis.Token next() throws IOException {
		try {
			org.apache.lucene.analysis.Token t = nextToken();
			return t;
	 ...
	 * {@link WordTokenizerConstants#tokenImage}.
	 */
	final public org.apache.lucene.analysis.Token nextToken()
		...
		if (token.kind == EOF) {
			...
			{
				if (true)
					return new org.apache.lucene.analysis.Token(token.image,
	...
	public WordTokenizer(CharStream stream) {
		token_source = new WordTokenizerTokenManager(stream);
		token = new Token();
		jj_ntk = -1;
		jj_gen = 0;
		for (int i = 0; i < 1; i++)
