...
//    in = new BufferedReader(new FileReader(theFile)); DWH 12-18-06
  ...
  {
    String ss=null;
    if (iNited == 0) // not initialized yet; never loads file unless it is needed
    {
      ss = initEnglishList();
      if (ss==null)
        iNited = 1;
      ...
      {
        iNited = 2; // can't be initialized, will default to Porter Stemmer, SHOULD show error message
//      if (topFrame==null)
//        JOptionPane.showMessageDialog(topFrame,ss+": Source stemming will default to Porter Stemmer","Java Workbench Engine",JOptionPane.WARNING_MESSAGE);
  ...
  {
    if (sSourceLanguage.equals("EN-US")) // DWH 12-12-06 added sSourceLanguage if
