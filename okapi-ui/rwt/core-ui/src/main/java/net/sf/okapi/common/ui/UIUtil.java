 ...
 ******************************************************************************/
	...
	public static int getPlatformType () {
		...
		if ( "rap".equals(SWT.getPlatform()) ) {
			// Currently we detect for OK/Cancel buttons order, Win/non-Win only 
			String ua = (String) RWT.getSessionStore().getAttribute("userAgent");
			if (ua.contains("Win")) return PFTYPE_WIN;
			else
				return PFTYPE_UNIX;
		}
		return -1; // Unknown
