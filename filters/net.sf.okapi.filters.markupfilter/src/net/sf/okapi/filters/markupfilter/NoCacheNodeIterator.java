 ...
 */
class NoCacheNodeIterator implements Iterator<Segment> {
	private final Segment segment;
	private int pos;
	private Tag nextTag;

	public NoCacheNodeIterator(final Segment segment) {
		this.segment=segment;
		pos=segment.getBegin();
		nextTag=segment.getSource().getNextTag(pos);
	...
	public Segment next() {
		...
		if (nextTag!=null) {
			...
			nextTag=segment.getSource().getNextTag(nextTag.getBegin()+1);
			if (nextTag!=null && nextTag.getBegin()>=segment.getEnd()) nextTag=null;
