...
import net.sf.okapi.filters.plaintext.ParametersPT;
...
public class PlainTextFilterTest {
	...
	public void testParameters() {
		...
		assertNotNull(rp);
		assertTrue(rp instanceof Parameters);
		
		Parameters rpp = (Parameters) rp;
		assertNotNull(rpp.rules);
		assertFalse(rpp.rules.isEmpty());
				
		// Check if defaults are set
		ParametersPT params = new ParametersPT(); 
		filter.setParameters(params);
		
		assertEquals(params.rule, PlainTextFilter.DEF_RULE);
		assertEquals(params.sourceGroup, PlainTextFilter.DEF_GROUP);
		assertEquals(params.regexOptions, PlainTextFilter.DEF_OPTIONS);
		
		// Load filter parameters from a file, check if params have changed
		URL paramsUrl = PlainTextFilter.class.getResource("/test_params1.txt");
		...
		paramsUrl = PlainTextFilter.class.getResource("/test_params2.txt");
		assertNotNull(paramsUrl);
